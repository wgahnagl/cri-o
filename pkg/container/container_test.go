...
var _ = t.Describe("Container", func() {
	...
	t.Describe("SpecAddMount", func() {
		It("should add the mount to the spec", func() {
			sut.SpecAddMount("test", "test", "test", "test", []string{"test"})
	...
	t.Describe("SpecAddAnnotations", func() {
		It("should set the spec annotations", func() {
			...
			Expect(sut.Spec().Config.Annotations[annotations.TTY]).To(Equal(fmt.Sprintf("%v", sut.Config().Tty)))
			Expect(sut.Spec().Config.Annotations[annotations.Stdin]).To(Equal(fmt.Sprintf("%v", sut.Config().Stdin)))
			Expect(sut.Spec().Config.Annotations[annotations.StdinOnce]).To(Equal(fmt.Sprintf("%v", sut.Config().StdinOnce)))
