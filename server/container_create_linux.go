...
func (s *Server) createSandboxContainer(ctx context.Context, ctr ctrIface.Container, sb *sandbox.Sandbox) (cntr *oci.Container, retErr error) {
	...
	if s.config.ReadOnly {
		...
		for target, mode := range mounts {
			if !isInCRIMounts(target, containerConfig.GetMounts()) {
				ctr.SpecAddMount(target, "tmpfs", "tmpfs", "", append(options, mode))
			}
		}
	}
	...
	if sb.HostNetwork() {
		...
		if !isInCRIMounts("/sys", containerConfig.GetMounts()) {
			...
			ctr.SpecAddMount("/sys", "sysfs", "sysfs", "/sys/fs/cgroup", []string{"nosuid", "noexec", "nodev", "ro"})
		}
	}

	if ctr.Privileged() {
		ctr.SpecAddMount("sys", "sysfs", "sysfs", "/sys", []string{"nosuid", "noexec", "nodev", "rw"})
		ctr.SpecAddMount("sys/fs/cgroup", "cgroup", "cgroup", "/sys/fs/cgroup", []string{"nosuid", "noexec", "nodev", "rw", "relatime"})
	}
	...
	ctr.SpecAddMount("/dev/shm", "bind", sb.ShmPath(), "/dev/shm", []string{"rw", "bind"})
	...
	if sb.ResolvPath() != "" {
		...
		ctr.SpecAddMount("/etc/resolv.conf", "bind", sb.ResolvPath(), "", []string{"bind", "nodev", "nosuid", "noexec"})
	}
	...
	if sb.HostnamePath() != "" {
		...
		ctr.SpecAddMount("/etc/hostname", "bind", sb.HostnamePath(), "", append(options, "bind"))
	}
	...
	if !isInCRIMounts("/etc/hosts", containerConfig.GetMounts()) && hostNetwork(containerConfig) {
		...
		ctr.SpecAddMount("/etc/hosts", "bind", "/etc/hosts", "", append(options, "bind"))
	}
	...
	for _, m := range mounts {
		ctr.SpecAddMount(m.Destination, "bind", m.Source, "", append(m.Options, "bind"))
	}
